// prooflayer-runner-lab is an isolated-lab harness for the one fixed built-in
// handler. It is not the production Runner entry point and accepts no command,
// executable, argument, or scenario input.
package main
...
func main() {
	if len(os.Args) != 1 {
		fmt.Fprintln(os.Stderr, "this fixed lab harness accepts no arguments")
		os.Exit(2)
	}
