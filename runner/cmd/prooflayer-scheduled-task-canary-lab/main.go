// prooflayer-scheduled-task-canary-lab is an isolated-lab harness for one fixed
// built-in Scheduled Task canary. It accepts no task name, trigger, action,
// command, argument, payload, or scenario input.
package main
...
func main() {
	if len(os.Args) != 1 {
		fmt.Fprintln(os.Stderr, "this fixed scheduled task canary harness accepts no arguments")
		os.Exit(2)
	}
	...
	result := runner.Execute(context.Background(), job.ExecutionRequest{
		...
		EnvironmentID:   labEnvironmentID,
		HostID:          labHostID,
		ScenarioID:      "windows-scheduled-task-canary",
